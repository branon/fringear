...
WebAudio.Sound.prototype._loadAndDecodeSound  = function(url, onLoad, onError){
  ...
  request.onload = function() {
    ...
    (function decode(audioData){
      context.decodeAudioData(audioData, function(buffer){onLoad(buffer)}, function(err){
        var newData = self._syncStream(audioData)
        if ( newData ){
          decode(newData);
        }else{
          onError(err);
...
WebAudio.Sound.prototype._syncStream = function (buffer){ // should be done by api itself. and hopefully will.
  ...
  if(i!=-1) {
    return buffer.slice(i); //carefull there it returns copy
  }
}
